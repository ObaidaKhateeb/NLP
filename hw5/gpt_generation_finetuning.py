...
#A function that tokenizes the training dataset
...
#A function that generates reviews
def reviews_generate(prompt, model, tokenizer, input_ids, attention_mask, max_length, temperature, top_k, top_p, repetition_penalty):
    output = model.generate(input_ids, attention_mask=attention_mask, max_length=max_length, temperature=temperature, top_k = top_k, top_p=top_p, repetition_penalty = repetition_penalty, do_sample=True, num_return_sequences=1)
    generated_text = tokenizer.decode(output[0], skip_special_tokens=True)
    return generated_text

#A function that saves the generated reviews to a txt file
...
def main():
    ...
    training_positive_args = TrainingArguments(output_dir= './results', evaluation_strategy = "no", learning_rate= 2e-5, per_device_train_batch_size = 8, per_device_eval_batch_size = 8, num_train_epochs = 3, weight_decay = 0.01)
    training_negative_args = TrainingArguments(output_dir= './results', evaluation_strategy = "no", learning_rate= 2e-5, per_device_train_batch_size = 8, per_device_eval_batch_size = 8, num_train_epochs = 3, weight_decay = 0.01)
    ...
    negative_reviews = []
    for i in range(5):
        positive_reviews.append(reviews_generate(prompt, model_positive, tokenizer_positive, input_ids_positive, attention_mask_positive, max_length=150, temperature=0.7, top_k=50, top_p=0.9, repetition_penalty=1.2))
        negative_reviews.append(reviews_generate(prompt, model_negative, tokenizer_negative, input_ids_negative, attention_mask_negative, max_length=150, temperature=0.7, top_k=50, top_p=0.9, repetition_penalty=1.2)) 
